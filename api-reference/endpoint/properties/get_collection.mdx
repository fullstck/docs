...
<ParamField query="sortMode" default="property" type="string">
  ...
  If `property` is chosen, `fromDate` and `toDate` parameters will be **_overridden_** so that the results will be sorted by the `createdAt` of each `property`.

  If `advert` is chosen: `fromDate` and `toDate` parameters will be **_overridden_** so that the results will be sorted by `createdAt` of each `advert`.
...
<ParamField query="toDate" type="date">
  Date until which the **_property_** is created  — inclusive. Example: `toDate=2021-05-07 12:42:42`
</ParamField>

<ParamField query="toExpiredAt" type="date">
  Date from which the **_advert_** is expired — inclusive.
  Example: `toExpiredAt=2021-05-07 12:42:42`
</ParamField>

<ParamField query="toUpdatedAt" type="date">
  Date until which the **_property_** is updated — inclusive. Example: `toUpdatedAt=2021-05-07 12:42:42`
</ParamField>
