...
<Warning>
Restrictings your endpoints to our IP addresses is highly recommended for security reasons:
...
<Note>
In order to use webhooks in production, you need to provide on [POST /searches](http://docs.melo.io/api-reference/endpoint/searches/create):
* an `endpointRecipient` to receive new `matches` in real time
* or/and an `eventEndpoint` to receive new `advert` `events` in real time.

Please read our [Concepts](https://docs.melo.io/concepts) to understand what is a `match`, an `advert` and an `event`.
...
<Tip>
If you only wish to receive new `properties` that meet your search criteria in real time -- those are called `matches` -- then you can just provide on [POST /searches](http://docs.melo.io/api-reference/endpoint/searches/create):
* an `endpointRecipient`
* and set the `notificationEnabled` parameter to `true`
</Tip>
