...
<ParamField query="sortMode" type="string"  default="property">
  ...
  If `property` is chosen, `fromDate` and `toDate` parameters will be overridden so that results will be sorted by the `createdAt` of each `property`. 
  
  If `advert` is chosen: `fromDate` and `toDate` parameters will be overridden so that results will be sorted by `createdAt` of each `advert`.
